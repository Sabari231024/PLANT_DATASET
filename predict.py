...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, prob in enumerate(pred):
            ...
            filtered_indices = [j for j in top5i if prob[j] >= conf_threshold]
            s += f"{', '.join(f'{names[j]} {prob[j]:.2f}' for j in filtered_indices)}, "


            # Write results
            text = '\n'.join(f'{prob[j]:.2f} {names[j]}' for j in filtered_indices)
            if save_img or view_img:
                print(text)
                annotator.text([10, 50], text, txt_color=(0,0,0))
            if save_txt:
